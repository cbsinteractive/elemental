...
def etree_to_dict(tree_node):
    ...
    if children != []:
        ...
        for sub_node in children:
            dic[tree_node.tag][sub_node.tag] = list(etree_to_dict(sub_node).values())[0]
    else:
...
class ElementalLive():
    ...
    def get_input_devices(self):
        ...
        events_xml = self.send_request(http_method="GET", url=events_url, headers=events_headers)
        # print("8**********", events_xml.text)
        devices_url = f'{self.server_ip}/devices'
        devices_headers = self.generate_headers(devices_url)
        devices_xml = self.send_request(http_method="GET", url=devices_url, headers=devices_headers)
